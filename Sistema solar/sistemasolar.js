let sistemaSolar = [
  ...
  planet1 == planet2 || planet2 == planet3 || planet1 == planet3
    ? (repetidos = false)// se cumple la variable repetido
    : (repetidos = true);// no se cumple la variable repetido
  ...
  if (nombres && repetidos) {
    for (i = 0; i < sistemaSolar.length; i++) {
      if (
        ...
        parrafo += `
          <div class="card" id="planeta${i}"> // ese i hace que sea variable el planeta para luego cambiarle el fondo
            <img src="${sistemaSolar[i].imagen}" alt="imagen de ${sistemaSolar[i].nombre}" />
              <div class="container">
                ...
                <button onclick="botoncillo()">+</button>
                </div>
          </div>
          `;
      }
  ...
  function botoncillo(){
    document.getElementById("parrafo").style.backgroundColor = sistemaSolar[i].fondo;
  }
  
  document.getElementById("div").innerHTML = parrafo;

/* ejercicio pero con función
let planet1 = window.prompt("Introduce un planeta: ").toLowerCase();
  let planet2 = window.prompt("Introduce otro planeta: ").toLowerCase();
  let planet3 = window.prompt("Introduce otro planeta más: ").toLowerCase();
  ...
  let nombresCorrectos = 0;
  let parrafo = "";
  funcion repetidos(planet1, planet2, planet3){
    let repetidos:
     planet1 == planet2 || planet2 == planet3 || planet1 == planet3
    ? (repetidos = false)// se cumple la variable repetido
    : (repetidos = true);// no se cumple la variable repetido
  ...
  funcion planetaexisite(planeta){
    let existe
    for(i=0; i sistemasolar.lenght; i++){
      if(sistemasolar[i].name.includes(planeta)){
        existe=true
        break
      else{
        existe=false
